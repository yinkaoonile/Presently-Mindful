...
const MOCK_MED_STATS = {
  totalMinutes: 125,
  currentStreak: 3,
  weeklyActivity: [
    { dayLabel: "M", didMeditate: true },
    { dayLabel: "T", didMeditate: true },
    { dayLabel: "W", didMeditate: false },
    { dayLabel: "T", didMeditate: true },
    { dayLabel: "F", didMeditate: false },
    { dayLabel: "S", didMeditate: true },
    { dayLabel: "S", didMeditate: true }
  ]
};

export default function Insights() {
  const { data: stats, isLoading: isStatsLoading, isError: isStatsError } = useGetStreak();
  const { data: medStats, isLoading: isMedStatsLoading, isError: isMedStatsError } = useGetMeditationStreak();

  const isLoading = isStatsLoading || isMedStatsLoading;
  const displayStats = stats || MOCK_STATS;
  const displayMedStats = medStats || MOCK_MED_STATS;
  const hasErrors = isStatsError || isMedStatsError;
  ...
  return (
    <div className="p-6 pt-12 flex flex-col gap-8 pb-32">
      <div className="text-center">
        ...
        {hasErrors && (
          <p className="text-xs text-accent mt-2">Showing sample data while connecting...</p>
        )}
      </div>
      ...
      >
        ...
        <h2 className="text-5xl font-display font-bold text-foreground mb-2">
          {displayStats.currentStreak} <span className="text-2xl text-muted-foreground font-medium">days</span>
        </h2>
        ...
        <div className="mt-8 pt-6 border-t border-border/50 flex justify-between px-4">
          <div className="text-center">
            ...
            <p className="font-bold text-lg">{displayStats.longestStreak}</p>
          </div>
          <div className="w-px bg-border/50" />
          <div className="text-center">
            <p className="text-sm text-muted-foreground mb-1">Total</p>
            <p className="font-bold text-lg">{displayStats.totalCheckins}</p>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {displayMedStats.totalMinutes} mindful minutes total
            </p>
          </div>
        ...
        <div className="flex justify-between items-center bg-white/50 rounded-2xl p-4 shadow-inner">
          {displayMedStats.weeklyActivity.map((day, i) => (
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Average: {displayStats.averageMood.toFixed(1)} / 5</p>
          </div>
        </div>
        
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={displayStats.weeklyMoods} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              <XAxis 
                dataKey="dayLabel" 
                axisLine={false} 
                tickLine={false} 
              ...
              <Bar dataKey="mood" radius={[6, 6, 6, 6]} maxBarSize={40}>
                {displayStats.weeklyMoods.map((entry, index) => (
                  <Cell 
                    key={`cell-${index}`} 
